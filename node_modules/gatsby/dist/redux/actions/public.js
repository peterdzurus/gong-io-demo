...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  if ("4" === `4`) {
    ...
    const fileContent = _fsExtra.default.readFileSync(page.component).toString();

    const isSSR = fileContent.includes(`exports.getServerData`) || fileContent.includes(`export const getServerData`) || fileContent.includes(`export function getServerData`) || fileContent.includes(`export async function getServerData`);
